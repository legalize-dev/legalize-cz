...
- g) obecně denaturovaným lihem líh s přísadou denaturačního prostředku (směsi), jehož složení je stanoveno nařízením Komise, kterým se upravuje vzájemné uznávání postupů úplné denaturace lihu pro účely osvobození od spotřební daně^17),

- h) zvláštně denaturovaným lihem líh denaturovaný jiným denaturačním prostředkem než uvedeným v písmenu g),
...
(1) Líh lze vyrábět a upravovat pouze v lihovaru nebo zvláštním lihovaru schváleném správním orgánem podle odstavce 2, a to jen právnickou nebo fyzickou osobou, které bylo, nejde-li o pěstitelské pálení, vydáno státní povolení podle zvláštního právního předpisu (dále jen „koncese“).

(2) Stanovisko k žádosti o koncesi vydává podle zvláštního právního předpisu^2)

- a) správce spotřební daně z lihu, jde-li o líh získaný způsobem uvedeným v § 2 odst. 1 písm. a) bodu 1 a jeho úpravu, nebo
...
(4) Správce spotřební daně z lihu nebo Ministerstvo průmyslu a obchodu vydá kladné stanovisko k žádosti o koncesi a schválí lihovar nebo zvláštní lihovar, jestliže jsou splněny podmínky uvedené v odstavci 3 a uspořádání výrobního zařízení lihovaru nebo zvláštního lihovaru splňuje podmínky uvedené v § 5.
...
(1) Konzumní líh, lihoviny a ostatní alkoholické nápoje lze vyrábět pouze v provozovně schválené správním orgánem podle odstavce 2, a to jen fyzickou nebo právnickou osobou, které bylo vydáno státní povolení podle zvláštního právního předpisu (dále jen „koncese“).

(2) Stanovisko k žádosti o koncesi vydává správce spotřební daně z lihu podle zvláštního právního předpisu.^2)
...
(4) Správce spotřební daně z lihu vydá kladné stanovisko k žádosti o koncesi a schválí provozovnu určenou k výrobě konzumního lihu, lihovin a ostatních alkoholických nápojů, jestliže jsou splněny podmínky uvedené v odstavci 3.
...
- b) při poruše měřidla jímat vyrobený líh do ověřených^6) nádrží zajištěných správcem spotřební daně z lihu a měřit způsobem, který stanoví prováděcí předpis,
...
(1) Druhy denaturačních prostředků, jejich minimální množství a účel použití denaturovaného lihu stanoví prováděcí předpis. Celní úřad v Plzni může po dohodě s Ministerstvem zdravotnictví v odůvodněných případech, není-li ohrožen cíl správy daní nebo jiný veřejný zájem, povolit použití jiného denaturačního prostředku nebo jiného minimálního množství denaturačního prostředku, než stanoví prováděcí předpis, nebo použití stanoveného denaturačního prostředku nebo jiného minimálního množství denaturačního prostředku pro jiný účel. V případě denaturace lihu použitého pro výrobu výrobků, které nejsou určeny pro lidskou konzumaci, jsou druhem denaturačních prostředků a jejich minimálním množstvím také druhy denaturačních prostředků a jejich minimální množství stanovené pro daný účel použití denaturovaného lihu právními předpisy státu, ve kterém byly tyto výrobky vyrobeny, pokud jsou vyrobeny
...
(2) Denaturační prostředek pro zvláštně denaturovaný líh může být provozovateli lihovaru dodán kupujícím; v tomto případě je kupující povinen předložit provozovateli lihovaru vzorek denaturačního prostředku a osvědčení o jeho druhu a koncentraci vydané podle zvláštního zákona.^7)
...
(3) Při zjišťování zásob lihu podle odstavce 1 se případný rozdíl mezi fyzickou zásobou a zásobou vykázanou v evidenci podle odstavce 2 snižuje o výši skutečně zjištěných ztrát, nejvýše však do výše norem ztrát lihu. Osvobození ztrát lihu od daně podle zvláštního zákona^12) je podmíněno vedením průkazné evidence, z níž je zřejmý základ pro výpočet normovaných ztrát. V odůvodněných případech může správce spotřební daně z lihu povolit výjimku z prováděcího předpisu upravujícího normy ztrát lihu.
...
- f) do výše desetinásobku sazby spotřební daně stanovené pro líh zvláštním zákonem^3) a připadající na množství lihu obsaženého ve výrobku, jde-li o přestupek podle odstavce 1 písm. c), f) a g).

##### § 17b

#### Společná ustanovení k přestupkům

(1) Přestupky podle § 17 odst. 1 písm. f) a g) a § 17a odst. 1 písm. f) a g) projednává Státní zemědělská a potravinářská inspekce (dále jen „Inspekce“)^15). Ostatní přestupky projednává správní orgán, který je správcem spotřební daně z lihu. Přestupky podle § 17 odst. 1 písm. h) a l) a § 17a odst. 1 písm. h) a j) může projednat na místě též orgán Policie České republiky.
...
Při výkonu působnosti podle tohoto zákona postupuje správce spotřební daně z lihu podle daňového řádu s výjimkou řízení o přestupcích.
