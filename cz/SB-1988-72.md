...
(1) Funkční spolehlivost roznětnic a ohmmetrů se musí před prvním použitím a dále po dobu používání nejméně jednou za tři měsíce přezkoušet^8) ve zkušebně určené podle zvláštního právního předpisu^8a) nebo u výrobce (dále jen „určená zkušebna“). Do doby používání se nezapočítávají tři měsíce následující po přezkoušení těchto přístrojů, pokud jsou skladovány za předepsaných podmínek. Roznětnice a ohmmetry, které při přezkoušení nevyhověly technickým podmínkám nebo které nebyly přezkoušeny v určené lhůtě, se nesmí používat.
