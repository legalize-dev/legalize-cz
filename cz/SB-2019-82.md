...
1) Odběratel doplní druh tabákového výrobku (cigarety, doutníky a cigarillos, tabák ke kouření, nebo zahřívané tabákové výrobky).

2) Do tohoto sloupce se zaznamenává kód tabákových nálepek v následujícím tvaru:

- a) první část kódu je tvořena velkým písmenem, které označuje druh tabákového výrobku (C pro cigarety, D pro doutníky a cigarillos, T pro tabák ke kouření, Z pro zahřívané tabákové výrobky),
...
- 4. zahřívaných tabákových výrobků množstvím tabáku v zahřívaných tabákových výrobcích v jednotkovém balení určeném k přímé spotřebě v gramech zaokrouhlené na jedno desetinné místo (toto množství se uvede s přesností na jedno desetinné místo) a počtem kusů v jednotkovém balení určeném k přímé spotřebě; tyto dvě náležitosti se oddělí lomítkem,
...
- 4. Zm6,2/20M (tabáková nálepka pro zahřívané tabákové výrobky o rozměru 16 mm x 32 mm pro jednotkové balení obsahující 6,2 g tabáku a dvacet kusů zahřívaných tabákových výrobků se sazbou daně, která je charakterizována písmenem M).
...
2) V případě, že odběratel v daném kalendářním roce disponoval tabákovými nálepkami se stejnými náležitostmi, ale odlišného rozměru, uvede vždy výsledky inventury za tyto dvě skupiny tabákových nálepek do bezprostředně následujících řádků tabulky, a to nejprve výsledky inventury za tabákové nálepky o rozměru 16 mm x 32 mm a následně za tabákové nálepky o rozměru 20 x 44 mm.
