...
- g) výmetné náplně jako komponenty střeliva uváděné na trh samostatně odpovídají technickým a bezpečnostním požadavkům stanoveným výrobcem pro daný účel použití střeliviny; úřad zahrne do zjištění výsledky posouzení shody, pokud u střeliviny bylo provedeno posouzení shody podle jiného právního předpisu, za předpokladu, že spolu se žádostí byly předloženy výsledky takového posouzení podle § 7 písm. d).
...
- g) návod pro provádění přebíjení střeliva, pokud jsou komponenty střeliva nutné k sestavení náboje nebo nábojky na trh uváděny samostatně,
