...
(1) Celní úřady provádějí kontrolu u výrobků navržených k propuštění do volného oběhu^10) podle přímo použitelného předpisu Evropských společenství, který upravuje požadavky na akreditaci a dozor nad trhem, týkající se uvádění výrobků na trh^11).

(2) Celní úřady nepropustí do režimu vývozu nebo pasivního zušlechťovacího styku nebezpečný výrobek, jehož vývoz je rozhodnutím Komise zakázán^12).
