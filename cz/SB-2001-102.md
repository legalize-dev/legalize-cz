...
- a) odebírat výrobek nebo vzorek ze série výrobku a podrobovat je kontrole bezpečnosti,

- b) kontrolovat, zda skutečné vlastnosti výrobku odpovídají požadavkům na bezpečnost,
...
- h) nařídit zpětné převzetí nebezpečného výrobku od spotřebitele, pokud tak výrobce nebo distributor neučiní podle § 5 odst. 7 věty první.
...
(4) Při postupu podle odstavce 2 písm. e) až h) nemají podané námitky nebo odvolání odkladný účinek.
...
(1) Celní úřady provádějí kontrolu u výrobků navržených k propuštění do celního režimu volného oběhu podle přímo použitelného předpisu Evropských společenství, který upravuje požadavky na akreditaci a dozor nad trhem, týkající se uvádění výrobků na trh^11).

(2) Celní úřady nepropustí do celního režimu vývozu nebo pasivního zušlechťovacího styku nebezpečný výrobek, jehož vývoz je rozhodnutím Komise zakázán^12).
...
- b) určení specializovaných míst pro kontroly určitého zboží při celním řízení, pokud bude rozhodnuto o nezbytnosti takového postupu.
